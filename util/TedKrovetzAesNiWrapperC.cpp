...
void pseudoRandomFunctionwPipelining(__m128i seed1, __m128i seed2, int gate, int numOfParties, __m128i* out)
{
	__m128i counter = { 0 };
	...
	for (int i = 0; i < numOfParties; i++)
	{
		temp[i] = _mm_set_epi32(0,0,gate,i);
	}
	myAES(temp, out, numOfParties, &aes_key);
...
void fixedKeyPseudoRandomFunctionwPipelining(__m128i seed1, __m128i seed2, int gate, int numOfParties, __m128i* out)
{
	__m128i counter = { 0 };
	...
	for (int i = 0; i < numOfParties; i++)
	{
		temp[i] = _mm_xor_si128(_mm_xor_si128(_mm_set_epi32(0,0,gate,i),dseed1),ddseed2);
	}
	myAES(temp, out, numOfParties, &fixed_aes_key);
	
	XORvectors2(out, temp,out,numOfParties);
}
